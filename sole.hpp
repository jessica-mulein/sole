...
uuid rebuild( const std::pair<uint64_t, uint64_t> &pair );
...
inline uint64_t get_time( uint64_t offset ) {
  typedef std::chrono::duration<uint64_t, std::ratio<1, 10000000>> hundrednano;
  auto uuid_time = std::chrono::time_point_cast<hundrednano>(std::chrono::system_clock::now());

  uuid_time += hundrednano( offset );

  // If the clock looks like it went backwards, or is the same, increment it.
  std::chrono::time_point<std::chrono::system_clock,hundrednano> last_uuid_time;
  if ( last_uuid_time <= uuid_time ) {
    uuid_time += hundrednano( 1 );
  }
  last_uuid_time = uuid_time;

  return uuid_time.time_since_epoch().count();
}

bool get_any_mac( std::vector<unsigned char> &_node );

#ifdef SOLE_DEFINE_IMPLEMENTATION
#if defined( _WIN32 )
#include <winsock2.h>
#include <process.h>
#include <iphlpapi.h>
...
#endif // SOLE_DEFINE_IMPLEMENTATION
...
inline uuid rebuild( const std::pair<uint64_t, uint64_t> &pair ) {
  uuid u;
  u.ab = pair.first;
  u.cd = pair.second;
  return u;
